...
### 근접 시야 (`objects`) — 앱의 장애물 뷰가 읽는 필드

`/perception/objects_summary`는 팔로워와 블랙박스를 위해 쓰인 메시지입니다.
클러스터 40개 × 측면 프로파일 64개 숫자 × 5 Hz = 초당 0.25 MB이고, 폰은 2 Hz
RFCOMM 시리얼입니다. 그래서 브릿지가 **그릴 수 있는 것만** 추려서 보냅니다.

```json
{"objects_frame":"lidar","objects_hidden":2,
 "objects":[{"id":11,"class":"person","raw_class":"person",
             "band_relation":"inside","x":5.0,"y":-1.1,
             "size":[0.55,0.62],"motion":"moving","speed_mps":0.9,
             "trail":[[5.0,1.6],[5.0,0.7],[5.0,-1.1]]}]}
```

- **`objects_frame`은 생산자가 말한 그대로**입니다. `pose_x`/`pose_y`가 쓰는
  map 프레임이 **아니고**, `obstacle_clusters.py`는 `lidar`,
  `hybrid_object_fusion.py`는 `chair_centre`라고 답합니다 (원점이 약 0.5 m
  차이). `null`이면 "생산자가 말하지 않음"이지 `lidar`가 아닙니다.
- 잘릴 때 버려지는 쪽은 **먼 쪽**입니다. 가까운 쪽을 버리면 곧 부딪힐 물체만
  숨기고 화면은 물체로 가득 찬 상태가 됩니다. `objects_hidden`은 표시 필터와
  개수 상한이 빼놓은 총합이고, 0이 아니면 "시야가 잘려 있다"고 말해야 합니다.
- **회랑(`band_relation` = `inside`/`overlap`) 안의 물체는 무슨 일이 있어도
  숨기지 않습니다.** 표시 필터는 지면 얼룩(높이 0.25 m 미만, 반사점 10개 미만)
  을 걷어내려고 있는 것이지, 무엇이 중요한지 정하려고 있는 게 아닙니다.
- **표시 필터는 링크로 나가는 사본에만 걸립니다.** `safety_gate`와 팔로워가
  읽는 `/perception/objects_summary` 원본은 손대지 않습니다.

#### `trail`은 브릿지가 직접 만듭니다

경로가 있어야 "저 사람이 회랑으로 **걸어 들어온** 것"과 "원래 거기 세워져
있던 것"을 구별할 수 있는데, 이 브랜치의 `obstacle_clusters.py`는 `trail`을
발행하지 않습니다. 그리고 그 파일은 **휠체어가 주행에 쓰는 파일이라 대시보드를
위해 고치지 않습니다.** 그래서 브릿지가 프레임을 이어붙여 스스로 만듭니다.

- 표본은 **map 프레임으로 저장하고 현재 pose 기준으로 되돌려서** 내보냅니다.
  물체 좌표는 의자 기준이라 그대로 쌓으면 **의자 자신의 주행 궤적**이 길가에
  세워진 모든 볼라드 밑에 그려지고, 보는 사람은 그걸 "군중이 뒤로 걸어온다"로
  읽습니다. 양쪽 끝을 같은 변환으로 통과시키면 주차된 물체는 점 하나로
  수렴하고 진짜 움직인 것만 선이 됩니다.
- **pose가 0.5초 이상 낡으면 경로를 아예 안 그립니다.** 위치추정이 흔들리는
  동안 휘어진 경로를 그리느니 점으로 보이는 편이 낫습니다.
- 같은 `id`가 1.5 m 넘게 튀면 이력을 버립니다. 트랙이 없을 때 `id`는 리스트
  인덱스라서 다음 프레임에 다른 물체에게 넘어갑니다.
- 생산자가 `trail`을 보내면 **그쪽을 우선**합니다 (`hybrid_object_fusion.py`,
  앱 브랜치의 생산자). 트래커의 전체 이력과 원산지 프레임을 갖고 있으니까요.
- 3 m 이동으로 스팬이 0.25 m에 못 미치는 경로는 버립니다. 주차된 물체의 중심점
  흔들림이지 이동이 아닙니다.

#### `class`는 표시용으로 한 번 교정됩니다

`classify()`의 vehicle 규칙에는 하한만 있고 상한이 없어서, 캠퍼스 담장이나
연석 라인이 "주차된 차"로 나옵니다 (2026-08-27 실측: vehicle 1254개 중 300개).
브릿지가 발자국 6.5 m 초과 또는 장단비 4.0 초과인 vehicle을 `obstacle`로
바꿔서 내보내고, 생산자의 원래 단어는 `raw_class`에 그대로 남깁니다.
`objects_counts`도 같은 라벨로 다시 셉니다 — "차량 1"이라고 써놓고 담장을
그리는 대시보드는 정작 중요한 순간에 믿기게 됩니다.

**`person`은 어떤 경우에도 바뀌지 않습니다.** `cluster_guard`가 읽는 유일한
라벨이고, 표시 로직이 그 단어에 닿을 수 있으면 안 됩니다.

### 스택이 이미 떠 있는지 (`stack_nodes_up`)

잡 러너는 **앱이 띄운 것만** 압니다. 키보드에서 올린 스택 — 대부분이 그렇습니다
— 앞에서 대시보드는 "대기 중"이라고 하고 [로컬 켜기]를 권했습니다. 그래서 ROS
마스터에게 직접 묻습니다: `stack_nodes_up` / `stack_nodes_total` /
`stack_running`. 판정이 아니라 **개수**인 이유는, 스택은 층으로 올라오고 조각
으로 죽기 때문입니다 — 5개 중 3개가 운전자가 가장 봐야 하는 상태입니다.
...
| `{"command":"stack_stop","confirm":true}` | **`stop_stack.sh` 실행** — 기동을 끝까지 되돌립니다 |
| `{"command":"arm_and_drive","confirm":true}` | 시동(65) → 컨트롤러 echo 대기 → **`go.sh`**. [주행 시작]도 베이스가 manual에 있으면 이 명령을 보냅니다 |
| `{"command":"route"}` | 경로/밴드 폴리라인 재전송 (연결 직후 앱이 한 번 보냅니다) |
