...
def test_recorded_stationary_person_eventually_allows_safe_bypass(monkeypatch):
    ...
    # cluster_guard names the bypass berth from the chair CENTRE; this
    # planner measures room OUTSIDE the padded rectangle, which already
    # accounts for 0.45 m of it. dwa_follower.bypass_obstacle_floor_m()
    # converts, and asserting the converted value rather than the raw
    # constant is what keeps the two conventions from silently drifting
    # back into a 0.80 m berth.
    assert follower.planner.calls[0]["obstacle_floor_m"] == (
        _module.bypass_obstacle_floor_m())
...
def test_committed_person_bypass_survives_lateral_arc(monkeypatch):
    ...
    assert all(
        call["obstacle_floor_m"] == _module.bypass_obstacle_floor_m()
...
def test_stationary_person_is_watched_from_plan_ahead_before_bypass(
    ...
    assert approach["obstacle_floor_m"] == (
        _module.bypass_obstacle_floor_m())
